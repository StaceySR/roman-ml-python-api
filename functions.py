...
def finding_similarity_percentage(dataset, query_image_filename, indexes, max):
    print(query_image_filename, indexes, indexes[-1], indexes.size, max)
    count = 0.0
    i = 1
    while i <= indexes.size and i <= max:
        filename = dataset[indexes[-i]].get_filename()
        print(filename)
        print(query_image_filename)
...
def find_similar_images(dataset, size, query_image):
    ...
    print("query_image_lbp_vector: ", query_lbp_hist)
    print("query_image+rgb_vector: ", query_rgb_hist)
